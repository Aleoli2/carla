...
class World(object):
    ...
    def restart(self):
        ...
        # This sensors aren't used by transfuser
    ...
    def tick(self, clock, socket, control):
        ...
        if self._architecture=="transfuser":
            ...
            socket.send_string(json.dumps(message),zmq.SNDMORE)
            socket.send(image)
        elif self._architecture == "aurova":
            image=self.rgb_camera._image.tobytes()
            ...
            socket.send(image,zmq.SNDMORE)
            socket.send(depth)
        elif self._architecture == "thinktwice":
            image=self.rgb_camera._image.tobytes()
            message["image_shape"]=self.rgb_camera._image.shape
            message["speed"]=speed
            message["steer"]=control.steer
            yaw=self.player.get_transform().rotation.yaw
            x, y =self.player.get_transform().location.x, self.player.get_transform().location.y
            message["input_x"]=x
            message["input_y"]=y
            message["accel"]=self.imu_sensor.accelerometer
            message["theta"]=yaw
            lidar=self.lidar_sensor._pointcloud.tobytes()
            message["lidar_shape"]=self.lidar_sensor._pointcloud.shape
    ...
    def destroy(self):
        ...
        if self._architecture=="transfuser" or self._architecture=="thinktwice": sensors.append(self.lidar_sensor.sensor)
        for sensor in sensors:
            if sensor is not None:
                sensor.stop()
                sensor.destroy()
        self.rgb_camera.destroy()
        if self._architecture=="aurova": self.depth_camera.destroy()
    ...
    try:
        ...
        while True:
            ...
            if socket_sub in socks and socks[socket_sub] == zmq.POLLIN:
                ...
                if(args.reverse): target_steer*=-1
                target_steer = np.clip(target_steer,-1.0,1.0)
                target_speed = float(message["speed"])
                if (args.architecture=="aurova" or args.architecture=="TCP"):
                    ...
                    if(args.reverse):
                        info[1]*=-1; info[3]*=-1
                elif (args.architecture=="transfuser" or args.architecture=="thinktwice"):
                    info=[float(message["speed"]),float(message["steer"])]
                    if(args.reverse): info[1]*=-1         
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='P',
        default="transfuser",
        help='Network arhitecture (transfuser, aurova, thinktwice or TCP)')
    argparser.add_argument(
        '--res',
    ...
    argparser.add_argument(
        '-r', '--reverse',
        action='store_true',
        help='Revert the output angle, the manual dataset was record incorrectly (TODO correct dataset)')
    args = argparser.parse_args()
    if args.architecture != "transfuser" and args.architecture != "TCP" and args.architecture != "aurova" and args.architecture!="thinktwice":
        print("ERROR: Neural network architecture should be transfuser, thinktwice, aurova or TCP")
