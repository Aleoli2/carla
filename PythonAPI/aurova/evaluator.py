...
class Evaluator(object):
    def __init__(self, carla_world : carla.World, args):
        ...
        self.offroad=args.offroad
        if args.offroad:
            for actor in self.world.get_actors():
                if actor.parent is not None and actor.parent.id == self.player.id:
                    if fnmatch(actor.type_id,'*lane_invasion'):
                        self.lane_invasion_sensor = LaneInvasionEvaluator(actor)
        ...
        if not args.no_collision:
            ...
            if self.collision_sensor is None or (args.offroad and self.lane_invasion_sensor is None):
